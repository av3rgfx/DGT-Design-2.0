...
description: Regole fondamentali di UX design (Hick, Fitts, Jakob, Prossimità, Miller, Doherty, Von Restorff, Serial Position, Peak-End, Zeigarnik, Prägnanz, Postel, Similarità, Connessione uniforme, Parkinson, Tesler, Goal-Gradient). Caricare SEMPRE prima di progettare, costruire, rivedere o criticare qualsiasi interfaccia utente - layout, navigazione, onboarding, form, impostazioni, dashboard, flussi interattivi, sistemi di design, token, componenti, wireframe, mockup, schermate, prototipi, approvazioni mobile - anche se l'utente non nomina esplicitamente la UX. Vale per proposte scritte, artboard, HTML/CSS/React e critiche di design. Per il prodotto DGT leggere anche reference/dgt-brief.md.
---
...
## Pre-flight (do this before producing anything)

1. Name the user's primary goal on this screen or flow in one sentence.
2. Name the single primary action. Everything else is secondary or hidden.
3. List what the user must NOT have to remember between screens.
4. If the project is DGT, read `reference/dgt-brief.md` in this skill folder: it carries the four questions every screen must answer, the anti-references, and the system constraints. They are constraints, not suggestions.
...
For each screen, component or flow you produced, answer in one line each:

- Which law justified the primary decision, and which law did you knowingly trade off?
- What is the one primary action, and is it visually dominant (Von Restorff) and physically reachable (Fitts)?
- What happens within 400 ms of every tap or click (Doherty)?
- What does the user see if it fails, and how do they recover without losing work (Postel)?
- Would a first-time user recognise every pattern from products they already know (Jakob)?
