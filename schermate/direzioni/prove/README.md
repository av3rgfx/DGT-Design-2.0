...
| `workflow.js` | `direzione-a.html?pagina=workflow`, `?pagina=dipartimento`, `mobile.html?schermata=10` | **versione 20**. Il perimetro delle consegne: le tre pillole del periodo, il titolo che cambia, «oggi» che ritorna esattamente la pagina della versione 19 (2 594 px su Sviluppo), le consegne dei giorni scorsi che sono richieste decise, la pillola «Da rifare», e il cerchio «cerca» che compare da solo quando la lista passa le dodici righe. I workflow: la pillola d'ingresso nell'intestazione (con la pagina Dipartimento che resta a 2 960 px e il rail a sei), l'elenco del dipartimento, il canvas (nodi, connettori, porte, il nodo del titolare senza avatar in tinta), il nodo che si apre con i suoi campi e il canvas che cresce, la firma anticipata che nasce spenta e si accende un workflow alla volta, i tre freni con i loro numeri, zero controlli inerti, quaranta, e la schermata 10 del telefono. Dalla **versione 27** quella schermata non è più una colonna di card ma **il canvas vero, in sola lettura**, e le sezioni che contavano `.m-wn` sono riscritte su `.wnode`: che Console e telefono chiamino la **stessa** funzione (`DGT_COMPONENTI.canvasWorkflow`, con il CSS nel foglio dei componenti e non più in quello della Console), che la sola lettura spenga cinque gesti contati (prese, «+», «×», trascinamento del nodo, «Riordina»), che si entri a «tutto dentro» — 278,4 / 910 = 0,306, col grafo che tocca tutti e due i fianchi a 0 e 348 px su 348 — che il tocco su un nodo porti a scala 1 centrato su quello (0,0 px dal centro) e ne apra i campi, che toccarlo di nuovo rimetta tutto dentro, che le etichette del contratto stiano **fuori** dal canvas a 10 px invece dei 4,3 che avrebbero dentro, che lo zoom valga anche sulla tab «L'ultima volta» (fino alla 26 era `ramo ? zoom : 1`), e i **due gesti del dito** — trascina-la-vista (160 px di dito = 128 px di disegno, cioè 160/1,25) e pinch (due dita a un terzo = ingrandimento a un terzo, col fondo a «tutto dentro»), costruiti con `TouchEvent` a mano perché `page.touchscreen` muove un dito solo. Tre righe rifanno il conto della **regola 42**: dopo lo zoom, dopo il trascinamento e dopo il pinch le posizioni dei nodi sono le stesse al pixel. Legge anche il modello: costo uguale alla somma dei passi, nessun passo rotto, almeno due conclusi, nodi = passi + 1. Dalla **versione 25** i tre freni del permesso in testa (decisione 71): il ramo terminale si **costruisce** col gesto vero (si tira dalla presa e si rilascia nel vuoto) perché nel grafo di partenza i rami terminali sono zero e un confronto fra zeri sarebbe verde senza provare niente; poi che con «chiedi prima di consegnare» quel ramo resti in azienda e con gli altri due permessi esca portando i tre freni, che la sezione dica quale delle due strade firma, che il tag stia nei 208 px del nodo e che il passo nato dal rilascio non si posi sopra un altro |
