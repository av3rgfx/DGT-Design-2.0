...
// Variabili: LOCAL_FONT_CSS (font locali), MOTION=no-preference (avatar in moto: fermarli con DGT_AVATAR_ORBE.fermo/fotogramma), SCALE=2, W=1440, CLICK="sel1|sel2" (clic prima della cattura).
...
(async () => {
  ...
  const ctx = await browser.newContext({ viewport: { width: +(process.env.W || 1440), height: 900 }, colorScheme: 'dark', reducedMotion: process.env.MOTION || 'reduce', deviceScaleFactor: +(process.env.SCALE || 1) });
